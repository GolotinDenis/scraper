...
def scrapSite():
    ...
    for cars in carsArray:
        car = Car( year=cars['data-attribute-year'], name=cars.findChild('h3').text.replace(cars['data-attribute-year'],''), desciption=cars.findChild('p',{'class':'description'}).text)
        car.save()
    return 'Success';
...
def mainPage():
    ...
    return render_template('main.html', data = response)
